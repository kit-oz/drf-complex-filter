...
class DynaimcFilterTests(APITestCase):
    ...
    def test_me(self):
        query = {
            "filters": json.dumps(
                {"type": "operator", "data": {"attribute": "user", "operator": "me"}}
            )
        }
    ...
    def test_not_me(self):
        query = {
            "filters": json.dumps(
                {
                    "type": "operator",
                    "data": {"attribute": "user", "operator": "not_me"},
                }
            )
        }
        self.client.force_authenticate(user=self.user)
    ...
    def test_me_unauthenticated(self):
        query = {
            "filters": json.dumps(
                {
                    "type": "operator",
                    "data": {"attribute": "user", "operator": "me", "value": ""},
                }
            )
        }
        response = self.client.get(self.URL, query, format="json")
    ...
    def test_not_me_unauthenticated(self):
        query = {
            "filters": json.dumps(
                {
                    "type": "operator",
                    "data": {"attribute": "user", "operator": "not_me", "value": ""},
                }
            )
        }
        response = self.client.get(self.URL, query, format="json")
